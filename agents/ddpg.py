...
class DDPG:
    ...
    def set_params(self, init_dict):
        self.batch_size = init_dict.get("batch_size", 256)
        self.update_rate = init_dict.get("update_rate", 20)
        self.discount = init_dict.get("discount", 0.995)
        self.alpha = init_dict.get("alpha", 0.0003)
        self.device = init_dict.get("device", "cpu")
        self.actor_lr = init_dict.get("actor_lr", 0.0001)
        self.critic_lr = init_dict.get("critic_lr", 0.0001)
    ...
    def act(self, state, epsilon=0.0, noise_amount=1.0):
        ...
        noise = np.random.normal(scale=noise_amount, size=actions.shape) # / 3 so 99% of noise falls between -1 and 1
    ...
    def learn(self, experience_batch):
        self.train_critic(experience_batch)
        self.train_actor(experience_batch)
...
class MemoryBuffer:
    def __init__(self, max_size=2**14):
